...
- 최신 로그 파일: `docs/evidence/metrics_run_output.txt`
- 케이스 구성: `resume_only/interview_only/plan_only/full` + 모호 질의(경계 조건) 혼합
- Prompt 비용 제어: `FEW_SHOT_MAX_EXAMPLES`로 직무별 Few-shot 주입 개수를 제한(0이면 생략)
- 구조화 실패 대비: `enforce_chat_response_contract` 기반 최소 스키마 강제로 UI/API 계약 안정성 유지
- 부분 실패 가시화: `ChatResponse.node_status(ok/degraded/skipped, error_code/detail)`로 노드별 상태를 증빙 로그에서 즉시 확인 가능
- 실행 예시(UTF-8 저장, 프로젝트 루트 기준):
