...
| 2) LangChain/LangGraph Multi-Agent | Multi-Agent 그래프 구성, Tool Calling, 세션 메모리 활용, LangGraph Checkpointer(`thread_id=session_id`) 기반 실행 상태 복원 | `src/workflow/engine.py`, `src/agents/tools.py`, `src/utils/memory.py` |
...
- Tool loop 고도화: 단일 호출이 아닌 최대 N회(기본 4회) 도구 재호출 루프 + 명시적 종료 지시(종료 토큰 의존 제거)
